...
describe("interactive input → game state", () => {
  ...
  // Per-widget state-mutation helpers below. The earlier clickToolbar() tests
  // (which routed a synthetic toolbar click → helper) were retired once the
  // native dispatch chain went end-to-end — see
  // test/runtime/native_dispatch.test.js for the full WM_LBUTTONDOWN+UP →
  // ring buffer → 0x42a830 widget dispatch → pause toggle, which is what the
  // browser path actually exercises today.
