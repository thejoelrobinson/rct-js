...
export function setLogicFlags16(cpu, result) {
  const value = result & 0xffff;
  cpu.eflags.ZF = value === 0 ? 1 : 0;
  cpu.eflags.SF = (value >>> 15) & 1;
  cpu.eflags.CF = 0;
  cpu.eflags.OF = 0;
}
export function setSubFlags16(cpu, a, b) {
  const left = a & 0xffff;
  const right = b & 0xffff;
  const result = (left - right) & 0xffff;
  cpu.eflags.ZF = result === 0 ? 1 : 0;
  cpu.eflags.SF = (result >>> 15) & 1;
  cpu.eflags.CF = left < right ? 1 : 0;
  cpu.eflags.OF = (((left ^ right) & (left ^ result)) >>> 15) & 1;
}
export function setAddFlags16(cpu, a, b) {
  const left = a & 0xffff;
  const right = b & 0xffff;
  const result = (left + right) & 0xffff;
  cpu.eflags.ZF = result === 0 ? 1 : 0;
  cpu.eflags.SF = (result >>> 15) & 1;
  cpu.eflags.CF = left + right > 0xffff ? 1 : 0;
  cpu.eflags.OF = ((~(left ^ right) & (left ^ result)) >>> 15) & 1;
}
...
export function stepInterp(cpu, addr) {
  reportFallback(cpu, addr, "instruction");
  if (!interpreterStep) throw new Error("lifted runtime interpreter step is not configured");
  cpu.regs.eip = addr >>> 0;
  interpreterStep(cpu);
}
...
let allowInterpreterFallback = true;
let fallbackObserver = null;
let interpreterStep = null;
export function configureLiftedRuntime(options = {}) {
  allowInterpreterFallback = options.allowInterpreterFallback !== false;
  fallbackObserver = typeof options.onFallback === "function" ? options.onFallback : null;
  interpreterStep = typeof options.step === "function" ? options.step : null;
}

function reportFallback(cpu, target, kind) {
  cpu.fallbackCount = (cpu.fallbackCount || 0) + 1;
  const event = {
    target: target >>> 0,
    kind,
    sourceEip: cpu.regs.eip >>> 0,
    registers: Object.fromEntries(["eax", "ebx", "ecx", "edx", "esi", "edi", "ebp", "esp"]
      .map((name) => [name, cpu.regs[name] >>> 0])),
    flags: Object.fromEntries(["CF", "ZF", "SF", "OF", "DF"].map((name) => [name, cpu.eflags[name] | 0])),
    trace: cpu.trace?.slice(-16).map((address) => address >>> 0) || [],
  };
  if (fallbackObserver) fallbackObserver(event);
  if (!allowInterpreterFallback) {
    throw new Error(`lifted runtime ${kind} fallback at 0x${(target >>> 0).toString(16)} from 0x${event.sourceEip.toString(16)}`);
  }
}
...
export function dispatch(cpu, target, fallthrough) {
  ...
  reportFallback(cpu, target, "dispatch");
