...
export function liftFunction(memory, funcAddr, endAddr, options = {}) {
  ...
  const approvedTargets = tableBase => {
    const count = options.staticJumpTables?.get?.(tableBase);
    if (count !== undefined && (!Number.isInteger(count) || count < 1 || count > 256)) {
      throw new Error(`invalid verified table length at 0x${tableBase.toString(16)}`);
    }
    const targets = [];
    for (let i = 0; i < (count ?? 256); i++) {
      const entryAddr = tableBase + i * 4;
      const inBounds = entryAddr >= 0 && entryAddr + 4 <= memory.length;
      const target = inBounds ? (memory[entryAddr] | (memory[entryAddr + 1] << 8) |
        (memory[entryAddr + 2] << 16) | (memory[entryAddr + 3] << 24)) >>> 0 : 0;
      const valid = target !== 0 && target < memory.length && (funcAddr < 0x400000 || target >= 0x400000);
      if (!valid) {
        if (count !== undefined) throw new Error(`invalid verified table target ${i} at 0x${tableBase.toString(16)}`);
        break;
      }
      targets.push(target);
    }
  ...
  while (worklist.length) {
    ...
    if (inst.control === "call_table") {
      if (!options.staticJumpTables?.has(inst.tableBase)) {
        ...
        const tableTargets = approvedTargets(inst.tableBase);
