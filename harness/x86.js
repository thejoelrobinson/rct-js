...
let activeDecodeHook = null;

export function setDecodeObserver(observer) {
  const previous = decodeObserver;
  decodeObserver = observer;
  return previous;
}

function invokeObservedHook(cpu, address, hook) {
  const previous = activeDecodeHook;
  activeDecodeHook = address >>> 0;
  try { return hook(cpu); }
  finally { activeDecodeHook = previous; }
}
...
export function step(cpu) {
  ...
  if (_eipHooksSize > 0) {
    ...
    if (hook) {
      cpu.hookCrossings = ((cpu.hookCrossings || 0) + 1) >>> 0;
      if (decodeObserver) invokeObservedHook(cpu, cpu.regs.eip, hook);
      else hook(cpu);
  ...
  if (decodeObserver) decodeObserver(ip >>> 0, activeDecodeHook, cpu);
  let opcode = m[ip];
  ...
  if (opcode >= 0x40 && opcode <= 0x47) {
    if (prefixOperandSize) {
      ...
      if (globalThis.__strictX86) cpu.eflags.OF = v === 0x8000 ? 1 : 0;
  ...
  if (opcode >= 0x48 && opcode <= 0x4f) {
    if (prefixOperandSize) {
      ...
      if (globalThis.__strictX86) cpu.eflags.OF = v === 0x7fff ? 1 : 0;
  ...
  // POP r/m32 (0x8f /0) — or POP r/m16 with 0x66, which moves esp by TWO.
  // The window paint dispatcher pops its saved DPI this way (0x5e1606:
  // `66 8f 05 98 fb 99 00`, five of them), so getting the width wrong here
  // desynchronises the stack for every window that is drawn.
  if (opcode === 0x8f) {
    const { operand, regField, len } = decodeModrm(cpu, ip + 1);
    if (regField !== 0) throw new Error(`unsupported 0x8f /${regField}`);
    if (prefixOperandSize) {
      const v = mem16(m, cpu.regs.esp);
      cpu.regs.esp = (cpu.regs.esp + 2) >>> 0;
      if (operand.kind === "reg") write16reg(cpu, operand.reg, v); else write16(m, operand.addr, v);
    } else {
      const v = mem32(m, cpu.regs.esp);
      cpu.regs.esp = (cpu.regs.esp + 4) >>> 0;
  ...
  if (opcode === 0xf3 || opcode === 0xf2) {
    ...
    let stringOpAt = ip + 1;
    let wordOverride = !!globalThis.__strictX86 && prefixOperandSize;
  ...
  if (opcode === 0x3a) {
    ...
    if (globalThis.__strictX86) cpu.eflags.OF = ((a ^ b) & (a ^ r)) >>> 7 & 1;
    cpu.regs.eip = (ip + 1 + len) >>> 0; return true;
  }
  ...
  if (opcode === 0xf6 || opcode === 0xf7) {
    ...
    if (regField === 2) { // NOT
      const a = readOp();
      writeOp(wide16 ? ((~a) & 0xffff) : wide ? ((~a) >>> 0) : ((~a) & 0xff));
      cpu.regs.eip = (ip + 1 + len) >>> 0; return true;
    }
    if (regField === 3) { // NEG
      const a = readOp();
      const r = wide16 ? ((-a) & 0xffff) : wide ? ((-a) >>> 0) : ((-a) & 0xff);
      writeOp(r);
      ...
      cpu.eflags.ZF = (r === 0) ? 1 : 0;
      cpu.eflags.SF = wide16 && globalThis.__strictX86 ? ((r >>> 15) & 1) : wide ? ((r >>> 31) & 1) : ((r >>> 7) & 1);
      cpu.regs.eip = (ip + 1 + len) >>> 0; return true;
    }
    ...
    if (regField === 4 && wide16 && globalThis.__strictX86) { // MUL ax, r/m16 → dx:ax
      const product = (cpu.regs.eax & 0xffff) * (readOp() & 0xffff);
      write16reg(cpu, 0, product);
      write16reg(cpu, 2, product >>> 16);
      cpu.eflags.CF = cpu.eflags.OF = (product >>> 16) !== 0 ? 1 : 0;
      cpu.regs.eip = (ip + 1 + len) >>> 0; return true;
    }
    if (regField === 5 && wide16 && globalThis.__strictX86) { // IMUL ax, r/m16 → dx:ax
      const a = ((cpu.regs.eax & 0xffff) << 16) >> 16;
      const raw = readOp() & 0xffff;
      const b = (raw << 16) >> 16;
      const product = a * b;
      write16reg(cpu, 0, product);
      write16reg(cpu, 2, product >> 16);
      cpu.eflags.CF = cpu.eflags.OF = product < -0x8000 || product > 0x7fff ? 1 : 0;
      cpu.regs.eip = (ip + 1 + len) >>> 0; return true;
    }
    ...
    if (regField === 6 && wide16 && globalThis.__strictX86) { // DIV — ax = (dx:ax) / r/m16, dx = remainder
      const divisor = readOp() & 0xffff;
      if (divisor === 0) throw new Error("divide by zero");
      const dividend = (cpu.regs.edx & 0xffff) * 0x10000 + (cpu.regs.eax & 0xffff);
      const q = Math.floor(dividend / divisor);
      const r = dividend % divisor;
      if (q > 0xffff) throw new Error("DIV r/m16 overflow");
      write16reg(cpu, 0, q);
      write16reg(cpu, 2, r);
      cpu.regs.eip = (ip + 1 + len) >>> 0; return true;
    }
    if (regField === 7 && wide16 && globalThis.__strictX86) { // IDIV — ax = (dx:ax) / r/m16, dx = remainder
      const raw = readOp() & 0xffff;
      const divisor = (raw << 16) >> 16;
      if (divisor === 0) throw new Error("divide by zero");
      const dividend = (((cpu.regs.edx & 0xffff) << 16) | (cpu.regs.eax & 0xffff)) | 0;
      const q = Math.trunc(dividend / divisor);
      const r = dividend - q * divisor;
      if (q < -0x8000 || q > 0x7fff) throw new Error("IDIV r/m16 overflow");
      write16reg(cpu, 0, q);
      write16reg(cpu, 2, r);
      cpu.regs.eip = (ip + 1 + len) >>> 0; return true;
    }
  ...
  if (opcode === 0x01) {
    ...
    if (prefixOperandSize) {
      ...
      if (globalThis.__strictX86) cpu.eflags.OF = (~(a ^ b) & (a ^ r)) >>> 15 & 1;
    } else {
  ...
  if (opcode === 0x3d) {                              // CMP eax, imm32 (or CMP ax, imm16 with prefix)
    if (prefixOperandSize) {
      const left = cpu.regs.eax & 0xffff;
      const right = mem16(m, ip + 1);
      if (globalThis.__strictX86) {
        const result = (left - right) & 0xffff;
        cpu.eflags.ZF = result === 0 ? 1 : 0;
        cpu.eflags.SF = result >>> 15;
        cpu.eflags.CF = left < right ? 1 : 0;
        cpu.eflags.OF = ((left ^ right) & (left ^ result)) >>> 15 & 1;
      } else setSubFlags(cpu, left, right);
      cpu.regs.eip = (ip + 3) >>> 0; return true;
    }
  ...
  if (opcode === 0xfe) {
    ...
    cpu.eflags.ZF = (r === 0) ? 1 : 0;
    cpu.eflags.SF = (r >>> 7) & 1;
    cpu.eflags.OF = a === (regField === 0 ? 0x7f : 0x80) ? 1 : 0;
    cpu.regs.eip = (ip + 1 + len) >>> 0; return true;
  }
  ...
  if (opcode === 0x83 && prefixOperandSize) {
    ...
    cpu.eflags.ZF = (r === 0) ? 1 : 0;
    cpu.eflags.SF = (r >>> 15) & 1;
    if (regField === 0) set16AddFlags(a, imm, r);
    else if (regField === 5 || regField === 7) set16SubFlags(a, imm, r);
    else set16Logic(r);
    cpu.regs.eip = (ip + 1 + len + 1) >>> 0; return true;
  }
  if (opcode === 0x83) {
  ...
  if (opcode === 0xc1 && prefixOperandSize && globalThis.__strictX86) {
    const { operand, regField, len } = decodeModrm(cpu, ip + 1);
    const count = mem8(m, ip + 1 + len) & 0x1f;
    const a = operand.kind === "reg" ? read16reg(cpu, operand.reg) : mem16(m, operand.addr);
    if (count === 0) { cpu.regs.eip = (ip + 1 + len + 1) >>> 0; return true; }
    let r;
    switch (regField) {
      case 0: { const c = count & 15; r = ((a << c) | (a >>> (16 - c))) & 0xffff; cpu.eflags.CF = r & 1; break; }
      case 1: { const c = count & 15; r = ((a >>> c) | (a << (16 - c))) & 0xffff; cpu.eflags.CF = (r >>> 15) & 1; break; }
      case 2: { const c = BigInt(count % 17); const full = BigInt(a) | (BigInt(cpu.eflags.CF) << 16n); const rotated = ((full << c) | (full >> (17n - c))) & 0x1ffffn; r = Number(rotated & 0xffffn); cpu.eflags.CF = Number((rotated >> 16n) & 1n); break; }
      case 3: { const c = BigInt(count % 17); const full = BigInt(a) | (BigInt(cpu.eflags.CF) << 16n); const rotated = ((full >> c) | (full << (17n - c))) & 0x1ffffn; r = Number(rotated & 0xffffn); cpu.eflags.CF = Number((rotated >> 16n) & 1n); break; }
      case 4: r = (a << count) & 0xffff; cpu.eflags.CF = (a >>> (16 - count)) & 1; break;
      case 5: r = (a >>> count) & 0xffff; cpu.eflags.CF = (a >>> (count - 1)) & 1; break;
      case 7: r = (((a << 16) >> 16) >> count) & 0xffff; cpu.eflags.CF = (a >>> (count - 1)) & 1; break;
      default: throw new Error(`unsupported 0x66 0xc1 /${regField}`);
    }
    if (operand.kind === "reg") write16reg(cpu, operand.reg, r); else write16(m, operand.addr, r);
    cpu.eflags.ZF = r === 0 ? 1 : 0;
    cpu.eflags.SF = (r >>> 15) & 1;
  ...
  if (opcode === 0x69) {
    const { operand, regField, len } = decodeModrm(cpu, ip + 1);
    if (prefixOperandSize && globalThis.__strictX86) {
      const raw = operand.kind === "reg" ? read16reg(cpu, operand.reg) : mem16(m, operand.addr);
      const value = (raw << 16) >> 16;
      const immediate = (mem16(m, ip + 1 + len) << 16) >> 16;
      write16reg(cpu, regField, Math.imul(value, immediate));
      cpu.eflags.CF = cpu.eflags.OF = +(value * immediate < -32768 || value * immediate > 32767);
      cpu.regs.eip = (ip + 1 + len + 2) >>> 0; return true;
    }
    const a = read32op(cpu, operand);
    const imm = mem32(m, ip + 1 + len);
    ...
    if (globalThis.__strictX86) cpu.eflags.CF = cpu.eflags.OF = +((a | 0) * (imm | 0) < -2147483648 || (a | 0) * (imm | 0) > 2147483647);
    cpu.regs.eip = (ip + 1 + len + 4) >>> 0; return true;
  }
  ...
  if (opcode === 0x6b) {
    ...
    if (prefixOperandSize && globalThis.__strictX86) {
      const raw = operand.kind === "reg" ? read16reg(cpu, operand.reg) : mem16(m, operand.addr);
      const value = (raw << 16) >> 16;
      const immediate = signExtend8(mem8(m, ip + 1 + len)) | 0;
      write16reg(cpu, regField, Math.imul(value, immediate));
      cpu.eflags.CF = cpu.eflags.OF = +(value * immediate < -32768 || value * immediate > 32767);
      cpu.regs.eip = (ip + 1 + len + 1) >>> 0; return true;
    }
  ...
  if (opcode === 0x0f) {
    ...
    if (op2 === 0xa3) {
      const { operand, regField, len } = decodeModrm(cpu, ip + 2);
      if (globalThis.__strictX86 && !prefixOperandSize && operand.kind !== "reg") {
        const index = cpu.regs[REG32[regField]] | 0;
        const address = (operand.addr + (index >> 5) * 4) >>> 0;
        cpu.eflags.CF = mem32(m, address) >>> (index & 31) & 1;
        cpu.regs.eip = (ip + 2 + len) >>> 0;
        return true;
      }
      const a = operand.kind === "reg" ? cpu.regs[REG32[operand.reg]] >>> 0 : mem32(m, operand.addr);
      const bit = cpu.regs[REG32[regField]] & 0x1f;
  ...
  if (opcode === 0xff) {
    ...
    if (subOp === 0 || subOp === 1) {
      ...
      if (prefixOperandSize) {
        ...
        if (operand.kind === "reg") write16reg(cpu, operand.reg, r); else write16(m, operand.addr, r);
        cpu.eflags.ZF = (r === 0) ? 1 : 0; cpu.eflags.SF = (r >>> 15) & 1;
        if (globalThis.__strictX86) cpu.eflags.OF = a === (subOp === 0 ? 0x7fff : 0x8000) ? 1 : 0;
      } else {
        const a = operand.kind === "reg" ? cpu.regs[REG32[operand.reg]] >>> 0 : mem32(m, operand.addr);
        const r = subOp === 0 ? ((a + 1) >>> 0) : ((a - 1) >>> 0);
        if (operand.kind === "reg") cpu.regs[REG32[operand.reg]] = r; else write32(m, operand.addr, r);
        cpu.eflags.ZF = (r === 0) ? 1 : 0; cpu.eflags.SF = (r >>> 31) & 1;
        if (globalThis.__strictX86) cpu.eflags.OF = a === (subOp === 0 ? 0x7fffffff : 0x80000000) ? 1 : 0;
      }
      cpu.regs.eip = (ip + 1 + len) >>> 0; return true;
    }
    ...
    if (subOp === 6) { // PUSH r/m32 — or PUSH r/m16 with 0x66 (esp -= 2)
      // The text renderer pushes a glyph's width this way (0x9baa75:
      // `66 ff b3 08 a5 99 00`) and pops it back with `66 58` (pop ax). A
      // 4-byte push against a 2-byte pop drifts the stack by 2 per character
      // drawn, and the eventual `ret` returns to garbage — which silently
      // ended the window paint part-way through its widget list.
      const { operand, len } = decodeModrm(cpu, ip + 1);
      if (prefixOperandSize) {
        const v = operand.kind === "reg" ? read16reg(cpu, operand.reg) : mem16(m, operand.addr);
        cpu.regs.esp = (cpu.regs.esp - 2) >>> 0;
        write16(m, cpu.regs.esp, v);
      } else {
...
export function runFunction(cpu, funcAddr, opts) {
  ...
  cpu.callDepth = 0;
  const hookCrossingsStart = cpu.hookCrossings || 0;
  if (typeof globalThis.__fnEntryHook === "function") {
    globalThis.__fnEntryHook(funcAddr >>> 0, cpu);
  }
    ...
    if (hook) {
      ...
      if (ret === RET_SENTINEL) {
        if (globalThis.__fnSteps) {
          ...
          if (cur) { cur.steps += 1; cur.hookCrossings += 1; cur.calls += 1; }
          else fm.set(funcAddr >>> 0, { steps: 1, decodedSteps: 0, hookCrossings: 1, calls: 1 });
        }
        return 1;
      }
    }
  ...
  if (globalThis.__fnSteps) {
    ...
    const hookCrossings = (cpu.hookCrossings || 0) - hookCrossingsStart;
    const decodedSteps = Math.max(0, totalSteps - hookCrossings);
    const cur = fm.get(funcAddr >>> 0);
    if (cur) {
      cur.steps += totalSteps;
      cur.decodedSteps += decodedSteps;
      cur.hookCrossings += hookCrossings;
      cur.calls += 1;
    } else {
      fm.set(funcAddr >>> 0, { steps: totalSteps, decodedSteps, hookCrossings, calls: 1 });
    }
  }
  return steps + 1;
}
