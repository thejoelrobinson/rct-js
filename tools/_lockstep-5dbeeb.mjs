#!/usr/bin/env node
// Per-call lockstep oracle for FUN_005dbeeb — the ride/vehicle per-sprite
...
//
// 0x5dbeeb is reached from inside the interpreter via the sprite-update walk
// at 0x5d952c (`call dword [edi*4 + 0x5d97b4]`, edi=4). The painter bridge
// installs an EIP HOOK at 0x5dbeeb. This tool REPLACES that hook with a
...
// hook + its __forceInterp5dbeeb leg are captured before we override.
//
//   TICKS=8 node tools/_lockstep-5dbeeb.mjs
...
const prodHook = getEipHook(0x5dbeeb);
...
const setRegs = (c, s) => {
  ...
  c.regs.ebp = s.ebp >>> 0; c.regs.esp = s.esp >>> 0; c.regs.eip = 0x5dbeeb;
};

setEipHook(0x5dbeeb, function lockstep(c) {
  calls++;
  save.set(bytes);
  const r0 = snapRegs(c);
  ...
  setRegs(c, r0);
  let jsThrew = null;
  if (process.env.AB_CONTROL) globalThis.__forceInterp5dbeeb = true;
  try { prodHook(c); } catch (e) { jsThrew = e; }
  finally { globalThis.__forceInterp5dbeeb = false; }
  afterJS.set(bytes);
