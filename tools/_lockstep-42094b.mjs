...
//
//   TICKS=3 node tools/_lockstep-42094b.mjs
...
const { paintBody42094b } = await import("../ported/auto/extra_paint_42094b.js");
...
const RANGES = () => {
  ...
  return [
    ["slots", head, 0x600],          // the cold loop can allocate many slots
    ["parent", 0x628928, 4],
    ["anchor", 0x6288fc, 4],
...
const lockstep = (cpu) => {
  ...
  const handled = paintBody42094b(heap, cpu);
  const sJS = snap(rg);
  const rJS = { ...cpu.regs };
  ...
  if (!handled) { /* fell back — interp ran once for both sides */ }
