...
// state.fnDispatch with translated functions, and calls FUN_00401000(heap) —
// the binary's WinMain-equivalent. Hybrid mode may install the development
// x86 oracle; pure-js mode never does.
//
...
import { promotedLifted as reviewedPromotedLifted } from "../generated/promoted-loader.js";
...
import { installPromotedLifted } from "./lifted-runtime.js";
...
import { FUN_004301c6 } from "../ported/auto/extra_window_4301c6.js";
...
import { tooltipInputRuntime, resetTooltipExact, showTooltipRuntime } from "../ported/auto/extra_tooltip.js";
 ...
 * @param {"hybrid"|"pure-js"} [opts.executionMode]
 * @param {{functions: object, entries: Array<number|string>}} [opts.promotedLifted]
 * @returns {{ heap: Heap, run: () => any }}
 */
export function createRuntime(opts) {
  const { dataBin } = opts;
  const executionMode = opts.executionMode || "hybrid";
  if (executionMode !== "hybrid" && executionMode !== "pure-js") {
    throw new Error(`invalid executionMode ${JSON.stringify(executionMode)}`);
  }
  ...
  // Verified lifted closures replace development interpreter shims and auto
  // translations. Hand-written overrides are installed below and therefore
  // retain highest precedence.
  // Reviewed lifted closures are production JS under the real startup path.
  // Keep the frozen soak on its original bridge bodies unless a caller
  // explicitly supplies a closure for an isolated runtime test.
  const promotedLifted = opts.promotedLifted || (globalThis.__realStartup || executionMode === "pure-js" ? reviewedPromotedLifted : null);
  if (promotedLifted) installPromotedLifted(heap, promotedLifted, executionMode);
  if (globalThis.__realStartup || executionMode === "pure-js") {
    state.fnDispatch.set(0x5e0cb6, FUN_005e0cb6);
    ...
    for (const [address, implementation] of [[0x42682d, FUN_0042682d], [0x5e585a, FUN_005e585a_exact],
      ...
      [0x5e2b52, tooltipInputRuntime], [0x430326, FUN_00430326], [0x5e2fa0, resetTooltipExact],
      [0x5e3652, showTooltipRuntime], [0x5e5b80, FUN_005e5b80_exact], [0x5e6a83, FUN_005e6a83_exact],
      [0x5e37e6, paintTooltipExact], [0x5e3861, FUN_005e3861_exact], [0x458678, FUN_00458678_exact],
      [0x5e412c, FUN_005e412c_exact], [0x5e4198, FUN_005e4198_exact],
      [0x4271da, FUN_004271da], [0x4271bb, FUN_004271bb],
      [0x5e5bd8, FUN_005e5bd8_exact], [0x5e43de, FUN_005e43de_exact]]) {
      const forceFlag = `__forceInterp${address.toString(16)}`;
      const previousDispatch = state.fnDispatch.get(address);
      state.fnDispatch.set(address, runtimeHeap => {
        if (address === 0x42b079 && regs.edi >>> 0 !== 0xffffffff) return previousDispatch(runtimeHeap);
        if (globalThis[forceFlag]) {
          if (state.__painterCpu) state.__painterCpu.eflags.DF = regs.df | 0;
          const result = callNative(address);
          const flags = state.__painterCpu.eflags;
          regs.cf = flags.CF; regs.zf = flags.ZF; regs.sf = flags.SF;
          regs.of = flags.OF; regs.df = flags.DF;
          return result;
        }
        return implementation(runtimeHeap);
      });
    }
    state.fnDispatch.set(0x4501b0, function FUN_4501b0_dispatch(runtimeHeap) {
      if (globalThis.__forceInterp4501b0) {
        if (state.__painterCpu) state.__painterCpu.eflags.DF = regs.df | 0;
        const result = callNative(0x4501b0);
        const flags = state.__painterCpu.eflags;
        regs.cf = flags.CF; regs.zf = flags.ZF; regs.sf = flags.SF;
        regs.of = flags.OF; regs.df = flags.DF;
        return result;
      }
      return FUN_004501b0(runtimeHeap);
    });
  }
  ...
  // Scenario-select paint callback. This CODESEG entry was absent from the
  // Ghidra inventory, so drawing the 128-row list previously fell through to
  // the interpreter. Keep the bridge path available for byte-diff oracles.
  if (globalThis.__realStartup) {
    state.fnDispatch.set(0x4301c6, function FUN_4301c6_dispatch(h) {
      if (globalThis.__forceInterp4301c6) return callNative(0x4301c6, []);
      return FUN_004301c6(h);
    });
  }
  ...
  return {
    ...
    runInit() {
      ...
      // REAL STARTUP (Task #13): when __realStartup is set, DON'T force-load a
      // scenario. The first tick of FUN_004385d8 (pre-run above as Phase H)
      // already ran its cb8==0 first-tick block, which at 0x43869a calls the
      // title-setup routine 0x438a1f (`or word [0x99a500],1` + MainOpen in title
      // mode) — i.e. the binary's own path opens the title menu (class 29) +
      // logo (class 39). The force-load below is the legacy hack that replaces
      // that title screen with an injected gameplay park; skip it for real boot.
      if (!globalThis.__realStartup) {
      ...
      // REAL STARTUP (Task #14): put the title screen into INPUT mode so the
      // menu (New Game / Load Game / Tutorial / Exit) is clickable.
      //
      // FUN_004385d8's per-tick body branches on DAT_00628cb9 (see
      // ported/auto/4385d8.js:186):
      //   cb9 == 0 -> run the input dispatch (FUN_004270f2) — widget clicks
      //               reach the title-menu event proc 0x429a65, whose New Game
      //               case (widget 0) calls 0x4304dd -> opens the scenario-
      //               select window (class 32).
      //   cb9 != 0 -> run the title INTRO/demo state machine (FUN_00438aac),
      //               which cycles demo parks (each re-runs the cb8==0 first-
      //               tick init, re-maps csg1.dat) and keeps bumping cb9, so
      //               the menu never accepts input.
      // The first-tick block set cb9 = 1 (the binary's intro entry). In retail
      // RCT the intro plays and then hands control to input mode (cb9 -> 0);
      // our demo-park loader isn't wired, so the intro state machine never
      // reaches that hand-off. Force cb9 = 0 to skip the (unwired) intro and
      // land on the interactive menu directly — the same spirit as skipFadeIn.
      // This also stops the demo-cycle churn (FUN_00438aac only runs when
      // cb9 != 0), so the title stays a stable class 0/29/39 window set.
      if (globalThis.__realStartup) heap.setU8(0x00628cb9, 0);

      // Run the binary's own research/ride-availability reset once, for the
      // same reason cb9 is forced above: the demo-park loader is not wired, so
      // nothing on the title takes the park-load path that would call it.
      //
      // FUN_004311e7 (reached only from 0x43054e, i.e. park load) zeroes the
      // 64-bit available-ride-type bit array at 0x87c3dc and the cached
      // -popcount of it at 0x87d7a2. Without it BOTH keep their link-time
      // values out of rct.exe's data section, and those DISAGREE: the array
      // reads 0xffffffff/0x0001ffff (49 bits) while the cache says -9.
      //
      // That inconsistency is expensive, because FUN_00424e0f's epilogue at
      // 0x424fca is a consistency retry — `call 0x4314ed; neg ax;
      // cmp ax,[0x87d7a2]; jne 0x424e0f` — that re-runs the whole map-aging
      // scan until the two agree. In a healthy game it never fires; here it
      // fired every tick and ran to our 1024-restart safety bound, issuing
      // ~10,000 tile invalidates per tick. Everything downstream followed:
      // the whole dirty-block grid went dirty, so the paint dispatcher
      // repainted the viewport ~36 times per tick, at ~1.86 MILLION
      // interpreter steps. MEASURED: title ticks 306ms -> 13ms (a 23x
      // speedup, ~3fps -> ~75fps) for one 0.1ms call.
      //
      // Retire this when the title demo-park load is wired — that path calls
      // 0x4311e7 itself, which is the point.
      if (globalThis.__realStartup) {
        const fn_4311e7 = state.fnDispatch.get(0x4311e7);
        if (typeof fn_4311e7 === "function") {
          try { fn_4311e7(heap); } catch (e) { /* non-fatal: worst case is the slow path */ }
        }
      }
      ...
      if (!globalThis.__realStartup && VP_VIEW_W > 0 && VP_VIEW_H > 0) {
    ...
    runTick(onProgress) {
      ...
      // RETIRED ON THE REAL-STARTUP PATH (ADDENDUM 138 item 1).
      //
      // The binary's own per-frame paint dispatcher is now wired and working:
      // 4385d8 -> 5e1653 -> 5e1210 (walk the dirty-block grid at 0x99ad63)
      // -> 5e12eb (build the clipped DPI at 0x99fb8c) -> 5e13d2 (draw each
      // window and its widgets). It was previously a no-op because 5e1210
      // dropped the AX/BX/DX/BP rect it passes to 5e12eb and 5e13d2 dropped the
      // EDI/AX arguments to the two paint calls; both are fixed and transcribed
      // from the binary now.
      //
      // Running BOTH is not additive, it is destructive. The pump walks the
      // pool every tick and repaints the viewport full-screen AFTER the
      // dispatcher has finished, so every window's chrome was wiped in the same
      // tick it was drawn — measured: the scenario-select dialog painted 80,296
      // pixels into the back buffer and 199,605 bytes of that buffer were
      // overwritten before the tick ended, which is exactly the "frame and
      // title bar with an entirely empty list" the bug report describes. The
      // pump also builds its temp viewport DPI at 0x99fb90, which is not spare
      // scratch at all — it is the clipX/clipY/clipW/clipH/pitch of the real
      // DPI struct that starts at 0x99fb8c.
      //
      // Kept for the legacy force-load path, which is what the frozen sc21 soak
      // baseline (5b79d5b5 / canary 7b14266) renders through and which keeps
      // the dispatcher gated off — see the gate note in ported/auto/5e1210.js.
      if (!globalThis.__realStartup
          && poolEnd > POOL_START && poolEnd < 0x009a013c + 256 * SLOT_STRIDE) {
        for (let slot = POOL_START; slot < poolEnd; slot += SLOT_STRIDE) {
          ...
          // SANITY-GATE the proc pointer before executing it (ADDENDUM 96).
          // rct.exe's code lives in [0x401000, 0x9c0000); anything outside that
          // is a CORRUPTED window struct, not a paint proc. This matters because
          // the UI branch below runs the pointer through callNative — i.e. it
          // EXECUTES it — so a corrupt pointer sends the interpreter into data
          // and it sprays writes across the heap. Observed live: a class-0
          // window whose proc had decayed from 0x42b079 to 0xb079 (high word
          // zeroed by a 16-bit value stored as u32), after which the framebuffer
          // progressively shredded into full-width colour bands over ~13k ticks.
          // Before the pump was taught to paint un-ported procs, a corrupt
          // pointer simply failed the fnDispatch lookup and was skipped; now the
          // range check is what keeps that safety property.
          if (wndProcAddr < 0x401000 || wndProcAddr >= 0x9c0000) {
            if (!globalThis.__warnedBadProc) {
              globalThis.__warnedBadProc = true;
              if (typeof console !== "undefined") {
                console.warn(`[harness] window slot 0x${slot.toString(16)} has a corrupt paint proc 0x${wndProcAddr.toString(16)} — skipping (see ADDENDUM 96)`);
              }
            }
            continue;
          }
          const fn = state.fnDispatch.get(wndProcAddr);
          // NB: do NOT skip here on a missing JS port. UI windows (below) are
          // painted through the interpreter via callNative(wndProcAddr) and do
          // NOT require `fn` — many dialog paint procs (e.g. scenario-select
          // cls 32, proc 0x4301c6) are unported. Only the viewport path uses
          // `fn(heap)` directly, so its JS-port requirement is re-checked there.
          ...
          if (viewportPtr === 0) {
            ...
            if (globalThis.__paintUiWindows && heap.u32(slot + 0x1c) !== 0) {
              ...
              try {
                regs.esi = slot >>> 0;
                // Per-window CLIPPED DPI (ADDENDUM 100).
                //
                // This used to pass the GLOBAL back-buffer DPI at 0x0099fb7c —
                // a full-screen 640x480 clip. Rendering looked fine, but a
                // paint proc told it owns the whole screen draws (and computes
                // buffer offsets) without any window bound, and these procs run
                // in the INTERPRETER, writing cpu.memory directly. Measured
                // consequence: the window pool at 0x9a013c (only 0x5c0 bytes
                // past this DPI struct) got scribbled — paint-proc pointers
                // truncated 0x42b079 -> 0xb079 and 0x429f6c -> 0x9f6c, which
                // killed the viewport repaint and the money readout.
                // A/B that proved it: with __paintUiWindows OFF the same run is
                // still intact at tick 4248; with it ON the pointers truncate.
                //
                // The binary never calls a window paint proc this way — its own
                // dispatcher hands each window a DPI clipped to that window's
                // rect. Build that here: bytes points at the window's top-left
                // in the back buffer, w/h are the window's size, and pitch is
                // the leftover bytes per row.
                const uiW = heap.u16(slot + 0x24), uiH = heap.u16(slot + 0x26);
                const uiX = (heap.u16(slot + 0x20) << 16) >> 16;
                const uiY = (heap.u16(slot + 0x22) << 16) >> 16;
                const backBuf = heap.u32(0x005f1fec) >>> 0;
                const pitch = heap.u32(0x005f2400) & 0xffff;
                const UI_DPI = 0x0099fba0;      // scratch, past the temp viewport DPI at 0x99fb90
                if (backBuf && pitch && uiW > 0 && uiH > 0
                    && uiX >= 0 && uiY >= 0 && uiX + uiW <= pitch) {
                  heap.setU32(UI_DPI + 0x00, (backBuf + uiY * pitch + uiX) >>> 0);
                  heap.setU16(UI_DPI + 0x04, uiX & 0xffff);
                  heap.setU16(UI_DPI + 0x06, uiY & 0xffff);
                  heap.setU16(UI_DPI + 0x08, uiW & 0xffff);
                  heap.setU16(UI_DPI + 0x0a, uiH & 0xffff);
                  heap.setU16(UI_DPI + 0x0c, (pitch - uiW) & 0xffff);
                  regs.edi = UI_DPI;
                } else {
                  regs.edi = 0x0099fb7c;        // fall back to the global DPI
                }
                // The binary selects a window paint proc's mode via AX (the low
                // word of EAX): every native paint-dispatch call site does
                // `mov ax, 0xffff; call [esi]` (see 0x5e366d / 0x5e3881 /
                // 0x5e4093). AX < 0 (sign bit set) = "draw the standard widget
                // frame" — the proc calls the generic widget painter 0x5e4400,
                // which then calls BACK into window+0 with AX = widget index
                // (>= 0) for any custom per-widget content. Without this, AX is
                // whatever stale value was in EAX; a POSITIVE AX routes multi-
                // message procs (e.g. scenario-select cls 32, proc 0x4301c6:
                // `or ax,ax / jns` at entry) into their custom-draw branch with
                // no widget frame drawn, so nothing appears. cls 29/39 procs
                // ignore AX (they only test edi != -1), so this is a no-op for
                // them; it is what makes cls 32 render its frame + widgets.
                regs.eax = ((regs.eax & 0xffff0000) | 0xffff) >>> 0;
                if (state.promotedLiftedAddresses.has(wndProcAddr) && typeof fn === "function") fn(heap);
                else callNative(wndProcAddr, []);
              } catch (e) {
          ...
          // Viewport path: this branch calls the JS-ported proc directly as
          // fn(heap), so it DOES require a JS port. (Moved down from the top of
          // the loop so UI windows above can paint via callNative without one.)
