...
// View cache: a real memory-mapped file returns a view backed by the file's
// shared pages, so mapping the SAME file region twice must return the same
// data — and, since our UnmapViewOfFile is a no-op (the buffer lives for the
// page's lifetime), re-using the same buffer is exact. Without this, the
// binary's re-init paths (e.g. the title-screen demo park cycling re-runs
// FUN_004385d8's first-tick block, which re-maps csg1.dat = 23 MB) leak a
// fresh buffer per re-map and exhaust the bump allocator after a couple of
// cycles. Keyed on the file identity (name) + offset + size, NOT the mapping
// handle — CreateFileMappingA mints a fresh handle on each re-init, so a
// handle key would never dedupe. First map of a region is a fresh alloc
// (byte-identical to the old behaviour); only re-maps hit the cache. During
// the byte-exact gameplay soak csg1.dat is mapped exactly once, so the cache
// never triggers there and the baseline is unaffected.
...
export function MapViewOfFile(heap, hFileMappingObject, dwDesiredAccess, dwFileOffsetHigh, dwFileOffsetLow, dwNumberOfBytesToMap) {
  ...
  const key = `${map.file.name || ""}:${dwFileOffsetLow >>> 0}:${want >>> 0}`;
