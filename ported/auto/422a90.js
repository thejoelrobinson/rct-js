...
//   python3 tools/disasm-va.py 0x422b20 0x4233f4        (16 case bodies)
//
...
//   0x422a97: jmp dword [ebx*4 + 0x422aa0]     (16-entry jumptable)
...
//   ebx = corner-case index (0..15 in practice; full 32-bit ebx indexes the
//         table in the binary — we replicate by reading the jumptable target
//         from the heap at (0x422aa0 + 4*ebx) mod 2^32 and matching it
//         against the 16 known case addresses, throwing on garbage so the
...
const TBL_422AA0 = 0x00422aa0; // jumptable base (16 dword entries)
...
export function FUN_00422a90(heap) {
  ...
  if (caseIdx === undefined) {
    // ebx outside 0..15 (would jump to garbage in the binary too). Never
    // observed in the soak; throw so the harness falls back to the
    // interpreter instead of silently diverging.
    throw new Error(
  ...
  if (totalDxAdd !== 0) {
    regs.edx = ((regs.edx & 0xffff0000) | ((dx0 + totalDxAdd) & 0xffff)) >>> 0;
  }
