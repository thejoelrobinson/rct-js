    ...
    for (; ebp > 0; ebp--) {
      ...
      if (!brokeToFb3 && reachedSurface) {
        ...
        if ((heap.u8(p + 5) & 0xe0) !== 0) {
          // goto LAB_00424f60 directly with current cx/ax
          secondWalk(heap, regs.ecx, regs.eax, ebp);
          didSecondWalk = true;
        } else {
          const bVar5a = (heap.u8(p + 5) & 0x1f) << 2;
          if (bVar5a > heap.u8(p + 2)) {
            // 0x424ecf: ja 0x424f13 — set [p+6]=1 then 5e56d3
            setOneAndInvalidate(heap, p);
            secondWalk(heap, regs.ecx, regs.eax, ebp);
            didSecondWalk = true;
          } else {
            // call 0x425432 (can-stand). regs.eax/ecx = (ax,cx) already.
...
function invalidate5e56d3(heap, esi, edi) {
  ...
  callNative(0x5e56d3, [esi >>> 0, edi >>> 0]);
}
