...
export function FUN_005e5301_exact(heap) {
  const kind = regs.eax & 0xff;
  const widgetIndex = regs.eax >>> 8 & 0xff;
  const identifier = regs.ebx & 0xffff;
  const widgetMode = (kind & 0x80) !== 0;
  const allOfClass = !widgetMode && (kind & 0x40) !== 0;
  const windowClass = widgetMode ? kind & 0x7f : allOfClass ? kind & 0xbf : kind;
  if (allOfClass) regs.eax = ((regs.eax & 0xffffff00) | windowClass) >>> 0;
  const end = heap.u32(0x9a1164);
  let window = 0x9a013c;
  for (; window < end; window += 0x178) {
    if (heap.u8(window + 0x174) !== windowClass || (!allOfClass && heap.u16(window + 0x30) !== identifier)) continue;
    const windowX = heap.u16(window + 0x20);
    const windowY = heap.u16(window + 0x22);
    if (widgetMode) {
      const widget = (heap.u32(window + 0x1c) + widgetIndex * 0x10) >>> 0;
      if (heap.u16(widget + 2) === 0xfffe) continue;
      markDirtyRect(heap,
        (windowX + heap.u16(widget + 2)) << 16 >> 16,
        (windowY + heap.u16(widget + 6)) << 16 >> 16,
        (windowX + heap.u16(widget + 4) + 1) << 16 >> 16,
        (windowY + heap.u16(widget + 8) + 1) << 16 >> 16);
    } else {
      markDirtyRect(heap, windowX << 16 >> 16, windowY << 16 >> 16,
        (windowX + heap.u16(window + 0x24)) << 16 >> 16,
        (windowY + heap.u16(window + 0x26)) << 16 >> 16);
    }
  }
  const difference = (window - end) >>> 0;
  regs.cf = window < end ? 1 : 0;
  regs.zf = difference === 0 ? 1 : 0;
  regs.sf = difference >>> 31;
  regs.of = ((window ^ end) & (window ^ difference)) >>> 31;
  return regs.eax >>> 0;
}
