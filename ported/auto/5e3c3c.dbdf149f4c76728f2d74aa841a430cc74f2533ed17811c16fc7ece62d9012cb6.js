...
import { createWindowExact } from "./extra_window_lifecycle.js";

const WINDOW_POOL = 0x009a013c;
const WINDOW_END = 0x009a1164;
const WINDOW_STRIDE = 0x178;

const word16 = value => value & 0xffff;
const signed16 = value => value << 16 >> 16;

function placementIsFree(heap, x, y, width, height, loose) {
  x = word16(x); y = word16(y); width = word16(width); height = word16(height);
  if (loose) {
    const quarterWidth = width >>> 2;
    const quarterHeight = height >>> 2;
    if (signed16(x) < signed16(-quarterWidth) ||
        signed16(x) > signed16(heap.u16(0x00971ed6) - quarterWidth * 2) ||
        signed16(y) < 30 ||
        signed16(y) > signed16(heap.u16(0x00971ed8) - quarterHeight)) return false;
  } else if (signed16(x) < 0 || signed16(y) < 30 ||
      signed16(word16(x + width)) > signed16(heap.u16(0x00971ed6)) ||
      signed16(word16(y + height)) > signed16(heap.u16(0x00971ed8))) return false;

  const right = signed16(word16(x + width));
  const bottom = signed16(word16(y + height));
  for (let window = WINDOW_POOL; window < heap.u32(WINDOW_END); window += WINDOW_STRIDE) {
    if (heap.u16(window + 0x32) & 1) continue;
    const left = signed16(heap.u16(window + 0x20));
    const top = signed16(heap.u16(window + 0x22));
    const windowRight = signed16(word16(heap.u16(window + 0x20) + heap.u16(window + 0x24)));
    const windowBottom = signed16(word16(heap.u16(window + 0x22) + heap.u16(window + 0x26)));
    if (right > left && signed16(x) < windowRight && bottom > top && signed16(y) < windowBottom) return false;
  }
  return true;
}

function automaticPlacement(heap, width, height) {
  const screenWidth = heap.u16(0x00971ed6);
  const screenHeight = heap.u16(0x00971ed8);
  const candidates = [
    [0, 0x20],
    [0, screenHeight - 0x24 - height],
    [screenWidth - width, 0x20],
    [screenWidth - width, screenHeight - 0x24 - height],
  ];
  for (const candidate of candidates) {
    if (placementIsFree(heap, candidate[0], candidate[1], width, height, false)) {
      return candidate.map(word16);
    }
  }

  for (let window = WINDOW_POOL; window < heap.u32(WINDOW_END); window += WINDOW_STRIDE) {
    if (heap.u16(window + 0x32) & 1) continue;
    const left = heap.u16(window + 0x20);
    const top = heap.u16(window + 0x22);
    const right = word16(left + heap.u16(window + 0x24));
    const bottom = word16(top + heap.u16(window + 0x26));
    const adjacent = [
      [right + 2, top], [left - width - 2, top],
      [left, bottom + 2], [left, top - height - 2],
      [right + 2, bottom - height], [left - width - 2, bottom - height],
      [right - width, bottom + 2], [right - width, top - height - 2],
    ];
  ...
  for (let window = WINDOW_POOL; window < heap.u32(WINDOW_END); window += WINDOW_STRIDE) {
    ...
    for (const candidate of adjacent) {
      if (placementIsFree(heap, candidate[0], candidate[1], width, height, true)) {
        return candidate.map(word16);
      }
    }
  }
  ...
  for (;;) {
    let occupied = false;
    for (let window = WINDOW_POOL; window < heap.u32(WINDOW_END); window += WINDOW_STRIDE) {
      if (heap.u16(window + 0x20) === x && heap.u16(window + 0x22) === y) {
        x = word16(x + 5);
        y = word16(y + 5);
        occupied = true;
        break;
...
export function FUN_005e3c3c_exact(heap, invoke) {
  const width = regs.ebx & 0xffff;
  const height = regs.ebx >>> 16;
  let x;
  let y;
  if ((regs.ecx & 0x80) !== 0) {
    regs.ecx = (regs.ecx & ~0x80) >>> 0;
    const windowClass = heap.u8(0x008dc08c);
    const identifier = heap.u16(0x008dc08e);
    let anchor = WINDOW_POOL;
    const end = heap.u32(WINDOW_END);
    while (anchor < end && (heap.u8(anchor + 0x174) !== windowClass || heap.u16(anchor + 0x30) !== identifier)) {
      anchor += WINDOW_STRIDE;
    }
    // 0x5e3c56 `call 0x5e3b2b` leaves the located window in ESI — or zero,
    // from the `xor esi, esi` at 0x5e3b55 when the search runs off the pool —
    // and 0x5e3c5b `mov edi, esi` copies it into EDI. Both survive the tail
    // jump at 0x5e3cc4 into 0x5e3f31, so they are part of this function's
    // contract, not scratch. The structural port searched the pool with a
    // local and left EDI holding whatever the caller passed;
    // test/runtime/window_placement.test.js caught it as EDI = 0x12345678
    // against the binary's anchor pointer.
    regs.esi = (anchor < end ? anchor : 0) >>> 0;
    regs.edi = regs.esi >>> 0;
    if (anchor < end &&
        signed16(heap.u16(anchor + 0x20)) < signed16(heap.u16(0x00971ed6) - 0x14) &&
        signed16(heap.u16(anchor + 0x20)) > -0x3c &&
        heap.u16(anchor + 0x22) < word16(heap.u16(0x00971ed8) - 0x14)) {
      x = heap.u16(anchor + 0x20);
      if (signed16(word16(x + width)) > signed16(heap.u16(0x00971ed6))) {
        x = word16(heap.u16(0x00971ed6) - 0x14 - width);
      }
      x = word16(x + 0x0a);
      y = word16(heap.u16(anchor + 0x22) + 0x0a);
    }
  }
  if (x === undefined) [x, y] = automaticPlacement(heap, width, height);

  // 0x5e3cc4 tail-jumps into 0x5e3f31 with EAX = y<<16 | x and EBX untouched.
  // Neither function clips x to the screen: 0x5e3f31 hands the raw value to
  // 0x452fce at 0x5e3ff3 (EBX = x + width/2, sign-extended) and stores it raw
  // at 0x5e4001. The structural port clamped x to 0 and shrank EBX first, so
  // an anchor near the left edge reached 0x452fce with x = 0 instead of the
  // binary's -49 — caught by test/runtime/window_placement.test.js.
  regs.eax = ((word16(y) << 16) | word16(x)) >>> 0;
  return createWindowExact(heap, invoke);
}

function FUN_005e3c3c_frozen(heap) {
...
export function FUN_005e3c3c(heap) {
  if (globalThis.__forceFrozen5e3c3c) return FUN_005e3c3c_frozen(heap);
  if (globalThis.__realStartup || state.executionMode === "pure-js") return FUN_005e3c3c_exact(heap);
  return FUN_005e3c3c_frozen(heap);
}
