...
export function FUN_004298a0(heap) {
  ...
  // 0x4299fb: `mov ebx, 0x520148` — the LOW half is the WIDTH (0x148 = 328) and
  // the high half is the HEIGHT (0x52 = 82), matching `mov [esi+0x24], ebx` in
  // 5e3f31. The two arguments were the wrong way round here, so the title menu
  // was created 82 wide x 328 tall instead of 328 x 82: a vertical sliver that
  // clipped away three of its four buttons and ran 226px off the bottom of a
  // 480-tall screen. That is the "title menu shows one button" report. The x
  // above is the tell — (screenW - 0x148) / 2 only centres a 328-wide window.
  regs.ebx = pack16(0x52, 0x148);
  regs.ecx = 0x21d >>> 0;
  regs.edx = 0x429a65 >>> 0;
  regs.ebp = 0x429ad5 >>> 0;
  FUN_005e3f31(heap);
  ...
  // 0x429a24: `mov ax,0; shl eax,0x10; mov ax,[0x971ed6]; sub ax,0xc8` — the
  // logo window is placed at x = screenW - 200, y = 0. This read screenH and
  // packed it as the Y, putting the window at (0, 280) instead of (440, 0).
  // 0x429a35: `mov ebx, 0x8500c8` — width 0xc8 = 200, height 0x85 = 133; the
  // same argument transposition as the menu above.
  const xLogo = (screenW - 0xc8) & 0xffff;
  regs.eax = pack16(0, xLogo);
  regs.ebx = pack16(0x85, 0xc8);
  regs.ecx = 0x227 >>> 0;
  regs.edx = 0x429ae1 >>> 0;
  regs.ebp = 0x429ae4 >>> 0;
  FUN_005e3f31(heap);
