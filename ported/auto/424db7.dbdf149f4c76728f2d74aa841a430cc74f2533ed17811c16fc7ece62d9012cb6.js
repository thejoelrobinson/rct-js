...
import { FUN_00424db7_frozen } from "./frozen_424db7.js";
...
export function FUN_00424db7_exact(heap) {
  const savedEsi = regs.esi >>> 0;
  const savedEdi = regs.edi >>> 0;
  const nesting = heap.u8(0x005f4948);
  ...
  if (regs.zf) {
    try {
      ...
      if (!regs.cf) {
        ...
        regs.cf = flags >>> 8 & 1;
        heap.setU16(flagsAddress, flags | 0x100);
  ...
  incrementByte(heap, 0x005f4948);
  return regs.eax >>> 0;
}

export function FUN_00424db7(heap) {
  if (globalThis.__forceFrozenToolbarBatch) return FUN_00424db7_frozen(heap);
  if (globalThis.__realStartup || state.executionMode === "pure-js") return FUN_00424db7_exact(heap);
  return FUN_00424db7_frozen(heap);
}
