...
export function FUN_0042f4be(heap) {
  ...
  if (true) {
    ...
    // asm 0x42f5e2: mov word ptr [esi + 0x16e], 0xffff  (esi = 0x6e3b80) — 16-bit.
    // 0x6e3cee/f0/f2 are three adjacent u16s; u32 stores stomped each other.
    heap.setU16(0x006e3cee, 0xffff);
    // asm 0x42f5eb: mov ax, word ptr [0x8ad1c2] / 0x42f5f1: mov word ptr [esi + 0x170], ax
    heap.setU16(0x006e3cf0, (heap.u8(0x008ad1c2)) >>> 0);
    // asm 0x42f5f8: mov ax, word ptr [0x8ad1c4] / 0x42f5fe: mov word ptr [esi + 0x172], ax
    // — a 16-bit store. NOTE: the LOAD is also mis-widthed (u8 should be u16); that is a
    // separate class and is left alone here (widening it alone also moves the hash).
    //
    // GATED to __realStartup only: 0x6e3cf2 is the last of the three adjacent u16s, so the
    // bogus u32 store also zeroed 0x6e3cf4/f5 — i.e. byte [esi+0x174], the main window's
    // number/classification field that 0x5e4400's widget painter compares against
    // [0x991f37]/[0x991f5a]. The frozen sc21 soak (5b79d5b5) was captured with that field
    // being force-zeroed on every park load, and letting it retain its real value changes
    // gameplay behaviour (hash e15a5b55). Only the real-startup path gets the faithful
    // 16-bit store; the frozen force-load path keeps the old zeroing behaviour.
    if (globalThis.__realStartup) {
      heap.setU16(0x006e3cf2, (heap.u8(0x008ad1c4)) >>> 0);
    } else {
