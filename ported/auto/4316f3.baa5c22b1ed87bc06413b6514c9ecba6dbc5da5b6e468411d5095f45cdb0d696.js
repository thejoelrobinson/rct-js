...
export function FUN_004316f3(heap) {
  ...
  // HAND-FIX (corrected): [0x5f96cc] is a 16-BIT field, not a byte. The bytes
  // at 0x43176c are `66 89 15 cc 96 5f 00` = `mov word ptr [0x5f96cc], dx`;
  // the earlier note here claimed `mov byte [0x5f96cc], cl`, which is not what
  // the binary contains. Truncating this strip-row index to 8 bits is why the
  // JS viewport painted nothing once it was driven by the binary's real paint
  // dispatcher (the synthetic pump had been hiding it by handing 4316f3 a
  // pre-compensated DPI). Its two readers below are widened to u16 to match.
  //
  // The shift is also `sar dx, cl` at 0x43175e — arithmetic, not logical.
  heap.setU16(0x005f96cc, (-(((heap.i16(0x005f96c8) >> (bVar1 & 0x1f)) - ((heap.i32(unaff_EDI + (2) * 4)) << 16 >> 16)) - ((heap.i32(unaff_EDI + (3) * 4)) << 16 >> 16))) & 0xffff);
  ...
  do {
    ...
    if ((heap.u16(0x00991f8c) & 1) != 0) {
      // 0x431845: `mov ebp, 0xa0a0a0a` — the pattern word 9b30bc fills with.
      // Dropped by the translator (pure register dataflow).
      regs.ebp = 0x0a0a0a0a;
    ...
    // 0x431878: `movzx ebp, byte ptr [0x8d7eb4]` — a BYTE index, then
    // `mov ebp, [ebp*4 + 0x628a3c]`. The translator read a DWORD at
    // 0x8d7eb4, so the table lookup landed outside 0x628a3c[] entirely and
    // the `cmp ebp, -1` gate answered from unrelated memory. EBP is also
    // left holding the loaded entry across both remaining calls.
    const _ebp = heap.i32((0x00628a3c + (heap.u8(0x008d7eb4) & 0xff) * 4)) | 0;
    regs.ebp = _ebp >>> 0;
    if (_ebp != -1) {
