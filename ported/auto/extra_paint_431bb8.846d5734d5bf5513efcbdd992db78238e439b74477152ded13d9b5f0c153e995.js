...
export function paintBody431bb8(heap, cpu, rotation) {
  ...
  if (dpi === 0) {
    ...
    // would `jng` out at this point. That exit is 0x432200 = `stc; ret`.
    cpu.eflags.CF = 1;
    return true;
  }
  ...
  // ADDENDUM 133 — every cull below is a `jle 0x432200` in the binary, and
  // 0x432200 is `stc; ret`. The CALLER observes that carry: the ride painter at
  // 0x5b1c52 does `call [ebp*4+0x431bb8]; jae +2; xor ebp,ebp; mov [0x99a4f0],ebp`.
  // So a culled paint must leave CF=1, and a successful one CF=0 with the slot
  // pointer still in EBP. Leaving CF stale made [0x99a4f0] read 0 forever.
  if (sx16to32(xdi) <= sx16to32(dpiX)) { cpu.eflags.CF = 1; return true; }
  if (sx16to32(xsi) <= sx16to32(dpiY)) { cpu.eflags.CF = 1; return true; }
  // dpi_x + dpi_w (16-bit signed) <= sprite_anchor_x → cull
  const dpiRight = (dpiX + dpiW) & 0xffff;
  if (sx16to32(dpiRight) <= sx16to32(heap.u16(DAT_6288FC))) { cpu.eflags.CF = 1; return true; }
  const dpiBottom = (dpiY + dpiH) & 0xffff;
  if (sx16to32(dpiBottom) <= sx16to32(heap.u16(DAT_6288FE))) { cpu.eflags.CF = 1; return true; }
  ...
  // ADDENDUM 125 — the bucket index is ROTATION-SPECIFIC. The port hardcoded
  // rotation 0's formula for all four variants, which is why rot 0 measured
  // heap-exact while rot 1/2/3 diverged (10838 and 42730 divergent pixels).
  // Recovered from the four jumptable bodies:
  //   rot0 0x431cfa: movzx edi,cx ; add di,ax ;
  //   rot1 0x431e86: movzx edi,cx ; sub di,ax ; add di,0x1000
  //   rot2 0x432016: movzx edi,cx ; add di,ax ; neg di ; add di,0x2000
  //   rot3 0x4321a7: movzx edi,AX ; sub di,CX ; add di,0x1000
  // (rot3 zero-extends AX and subtracts CX — the operands are swapped
  // relative to rot1, which is why a search anchored on `movzx edi,cx`
  // finds only three of the four.)
  // All four then share: jns / xor di,di (signed-16 < 0 -> 0), shr di,5,
  // cap at 0xff. NOTE the sign test happens AFTER the bias is added.
  let bktDi;
  if (rotation === 0) {
    bktDi = ((cx & 0xffff) + ax) & 0xffff;
  } else if (rotation === 1) {
    bktDi = ((((cx & 0xffff) - ax) & 0xffff) + 0x1000) & 0xffff;
  } else if (rotation === 2) {
    bktDi = ((-(((cx & 0xffff) + ax) & 0xffff)) + 0x2000) & 0xffff;
  } else {
    bktDi = ((((ax & 0xffff) - cx) & 0xffff) + 0x1000) & 0xffff;
  }
  ...
  // === and ax, ax  (clears CF) ; ret ===
  // The earlier note here -- "no observable effect on caller (421d2c discards
  // eflags)" -- was wrong: 421d2c is not the only caller. The ride/track
  // painters call this table directly and read BOTH results:
  //   0x5b1c52  call dword ptr [ebp*4 + 0x431bb8]
  //   0x5b1c59  jae 0x5b1c5d          ; CF==0 -> keep ebp
  //   0x5b1c5b  xor ebp, ebp          ; CF==1 -> ebp = 0
  //   0x5b1c5d  mov dword ptr [0x99a4f0], ebp
  // `and ax,ax` is a logical op, so it CLEARS CF; and EBP still holds the slot
  // pointer loaded from [0x5f96e8] at entry. Both must be reproduced or the
  // caller stores 0 -- which is exactly the sc3 divergence at 0x99a4f0.
  cpu.eflags.CF = 0;
  cpu.regs.ebp = slotPtr >>> 0;
  return true;
}
