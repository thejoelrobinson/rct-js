...
function paintCell(heap, ax, cx, setup) {
  regs.eax = ax;
  regs.ecx = cx;
  if (setup) FUN_004367cb(heap);
  regs.eax = ax;
  regs.ecx = cx;
  FUN_00444820(heap);
}

function walkRotatedDiamond(heap, rotation) {
  // Unlike the rotation-0 JS dispatch, these entries are reached through a
  // native call from 436b2a. Its EDI is the clipped DPI for this strip; using
  // [0x981ef8] here skips that first clip and paints a different diamond.
  const dpi = regs.edi >>> 0;
  const ay = (heap.u16(dpi + 6) - 0x10) & 0xffe0;
  let bx = heap.u16(dpi + 4) & 0xffe0;
  bx = (((bx << 16) >> 17) & 0xffff);
  ...
  let rows = ((heap.u16(dpi + 0xa) + 0x460) & 0xffff) >>> 5;
  while (rows-- > 0) {
    paintCell(heap, ax, cx, true);
    if (rotation === 1) {
      ax = (ax - 0x20) & 0xffff;
      cx = (cx - 0x20) & 0xffff;
      paintCell(heap, ax, cx, false);
      cx = (cx + 0x20) & 0xffff;
      paintCell(heap, ax, cx, true);
      cx = (cx + 0x20) & 0xffff;
      ax = (ax + 0x20) & 0xffff;
...
export function FUN_extra_paint_436bc3(heap) { walkRotatedDiamond(heap, 1); }
export function FUN_extra_paint_436c3d(heap) { walkRotatedDiamond(heap, 2); }
export function FUN_extra_paint_436cb3(heap) { walkRotatedDiamond(heap, 3); }
