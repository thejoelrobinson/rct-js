...
export function FUN_005e1fdd(heap) {
  ...
  //
  // LAB_005e21c0 (asm 0x5e21c0..0x5e2224). FOUR asm sites jump here:
  //   0x5e20f5  je 0x5e21c0   (playback: replay word == 0)
  //   0x5e2117  je 0x5e21c0   ([0x5ebee4] == 0)
  //   0x5e212d  je 0x5e21c0   (test al,0x80 == 0)
  //   0x5e2152  je 0x5e21c0   (test [0x5f1288],0x80 == 0)
  // The translator could express only the third (it inlined the block); the
  // other three became `cx=4; return 0` soft early-returns, so `call 0x403b39`,
  // the `btr [0x991f30],5` and `[0x99fdf4] = 0x80000000` never ran and the
  // caller got coordinate 0 instead of the cached drag coords. Hoisted into a
  // local so all four jump sites reach the real block.
  const LAB_005e21c0 = () => {
    (regs.eax = FUN_00403b39(heap));
    heap.setU32(0x00991f30, (heap.u32(0x00991f30) & 0xffffffdf) >>> 0);
    if (heap.u8(0x0099c16b) != 1) {
      // asm 0x5e21f6
      let uVar = ((heap.u32(0x005f128c)) >>> 0);
      if (heap.u8(0x0099c16b) == 2) {
        uVar = (((regs.eax = FUN_0042d637(heap))) >>> 0);
        (regs.eax = FUN_0042d637(heap));
      }
    ...
    // asm 0x5e21d6
    const sVar = (((regs.eax = FUN_0042d60a(heap))) & 0xffff);
  ...
  if (heap.u8(0x0099c16b) == 1) {
    ...
    if (sVar1 == 0) {
      return LAB_005e21c0(); // asm 0x5e20f5: je 0x5e21c0
    }
  } else {
    if (heap.u8(0x0099c16b) == 2) {
      (regs.eax = FUN_0042d637(heap));
    ...
    if (heap.u32(0x005ebee4) == 0) {
      return LAB_005e21c0(); // asm 0x5e2117: je 0x5e21c0
    }
  }
  if (heap.u8(0x0099c16b) == 1) {
    uVar3 = (((regs.eax = FUN_0042d60a(heap))) >>> 0);
    if ((uVar3 & 0x80) == 0) {
      return LAB_005e21c0(); // asm 0x5e212d: je 0x5e21c0
    }
  } else {
    if (heap.u8(0x0099c16b) == 2) {
      (regs.eax = FUN_0042d637(heap));
    ...
    if ((heap.u32(0x005f1288) & 0x80) == 0) {
      return LAB_005e21c0(); // asm 0x5e2152: je 0x5e21c0
    }
  }
  if (heap.u8(0x0099c16b) != 1) {
