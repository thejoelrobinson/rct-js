// Auto-translated from Ghidra C by tools/c-to-js/translate.js.
// Source: decompiled/c/44149a.c
// Edit by hand only after diff-test passes — re-running the translator will overwrite.

/** @typedef {import("../../runtime/heap.js").Heap} Heap */

import { CONCAT44 } from "../../runtime/ghidra-builtins.js";
import { regs } from "../../runtime/regs.js";
import { FUN_0043c60b } from "./43c60b.js";
import { FUN_0044142c } from "./44142c.js";
import { FUN_00441452 } from "./441452.js";
export function FUN_0044149a(heap) {
  let uVar1 = 0;
  let in_EAX = regs.eax >>> 0;
  let in_ECX = regs.ecx >>> 0;
  let in_EDX = regs.edx >>> 0;
  let uVar2 = 0;
  let iVar3 = 0;
  uVar1 = ((heap.u32((0x00991f8e) + ((((((in_EAX & 0xfe0) << 2) & 0xffff) | ((in_ECX >>> 5) & 0xffff) & 0x7ff) & 0xffff)) * 4)) & 0xffff);
  while (uVar1 != 0xffff) {
    uVar2 = ((((uVar1) >>> 0)) >>> 0);
    iVar3 = ((uVar2 * 0x100) >>> 0);
    if (((heap.u32((0x00743b9c) + (iVar3) * 4) == 4) && (heap.u32((0x00743bbf) + (iVar3) * 4) == 8)) && (((in_EDX) << 16 >> 16) == heap.u32((0x00743ba6) + (uVar2 * 0x80) * 4))) {
      (regs.eax = FUN_0044142c(heap));
      heap.setU32(((0x00743bbf) + (iVar3) * 4), (5) & 0xffffffff);
      (regs.eax = FUN_00441452(heap));
      uVar1 = ((heap.u32((0x00743ba4) + (uVar2 * 0x80) * 4)) & 0xffff);
      heap.setU16((0x00743bc6 + iVar3), ((heap.u32((0x00743ba2) + (uVar2 * 0x80) * 4) & 0xffe0) + 0x10) & 0xffff);
      heap.setU16((0x00743bc8 + iVar3), ((uVar1 & 0xffe0) + 0x10) & 0xffff);
      heap.setU32(((0x00743bca) + (iVar3) * 4), (5) & 0xffffffff);
      (regs.eax = FUN_0043c60b(heap));
    }
    uVar1 = ((heap.u32((0x00743b96) + (uVar2 * 0x80) * 4)) & 0xffff);
  }
  return 1;
}
