...
import { hitTestWindowExact, runSelectionEntry } from "./extra_ui_selection.js";
import { regs } from "../../runtime/regs.js";
import { FUN_005e3874 } from "./5e3874.js";
export function FUN_005e3ace(heap) {
  if (globalThis.__realStartup || state.executionMode === "pure-js") return runSelectionEntry(heap, 0x5e3ace, hitTestWindowExact);
  ...
  LAB_005e3ad4: do {
    ...
    if ((heap.u16((puVar4 + -0x146)) & 0x20) != 0) {
      // asm 0x5e3b0e: `push edi; call 0x5e3874; pop edi`, with ESI = the slot
      // pointer and AX/BX = the click coords. FUN_005e3874 preserves
      // EAX/EBX/ECX (push/pop at 0x5e3874-77 / 0x5e3883-84 / 0x5e38f2-f3) and
      // returns the hit widget index in EDX (-1 = no hit), EDI = descriptor.
      // Ghidra modelled EDX:EAX as one 64-bit `uVar5` and tested its high half
      // with `>>> 0x20`, which in JS is `>>> 0` — so the test was never true
      // and the `je 0x5e3ad4` below was dead code. It also returned undefined
      // into regs.eax, zeroing in_EAX for every later pool slot.
      regs.esi = puVar3 >>> 0;
      regs.eax = in_EAX >>> 0;
      regs.ebx = unaff_EBX >>> 0;
      FUN_005e3874(heap);
      unaff_EDI = regs.edi >>> 0;
      if ((regs.edx | 0) == -1) {
        // asm 0x5e3b18: `je 0x5e3ad4` — the widget hit-test found nothing in
        // THIS slot, so keep walking the pool (esi already == puVar3, and
        // 0x5e3ad4 is `sub esi,0x178`). The translator emitted a soft
        // early-return here, which reported "no window under the cursor" for
        // the whole pool the moment one hit-test-capable slot missed.
