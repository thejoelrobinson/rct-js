...
import { callPromotedLiftedFromCpu } from "../../runtime/lifted-runtime.js";
...
function corner432204(heap, cpu, runFunction, o, dxPaint) {
  ...
  cpu.regs.ebp = rot;
  callBridge(heap, cpu, runFunction, heap.u32((TBL_432204 + (rot & 3) * 4) >>> 0));
}

// 0x422801..0x422a9d — draw the flagged terrain cliff corners, then enter
// the already-transcribed corner-height dispatch. The same sequence occurs
// after the water block and on ordinary dry terrain; keep it one routine so
// both paths retain identical register and palette side effects.
function cliffCornerBlock(heap, cpu, runFunction) {
  const savedEax = cpu.regs.eax >>> 0;
  const savedEbx = cpu.regs.ebx >>> 0;
  const savedEcx = cpu.regs.ecx >>> 0;
  const savedEdx = cpu.regs.edx >>> 0;
  const savedEsi = cpu.regs.esi >>> 0;
  const mask = heap.u8((savedEsi + 7) >>> 0) & 0xf;
  let al = mask;
  cpu.regs.eax = (savedEax & 0xffffff00) | al;

  if (mask !== 0) {
    heap.setU8(0x00991f78, 8);
    al = ((mask << (savedEcx & 0xff)) & 0xff);
    al = (al | (al >>> 4)) & 0xff;
    const bl = savedEbx & 0xff;
    ...
    const corners = [
      {
        img: () => {
          if ((bl & 1) === 0) return { image: (bl & 8) === 0 ? 0x9238 : 0x923a, lift: 0 };
          if ((bl & 8) === 0) return { image: 0x923c, lift: 0 };
          if ((bl & 0x10) === 0) return { image: 0x9238, lift: 0x10 };
          if ((bl & 4) !== 0) return { image: 0x923a, lift: 0x10 };
          if ((bl & 2) !== 0) return { image: 0x923c, lift: 0x10 };
          return { image: 0x9238, lift: 0x10 };
        },
        regs: { al: 1, cl: 0x1f, di: 0x1e, si: 1, ah: 9, e8: 1, ea: 0x1f },
      },
      {
        img: () => {
          if ((bl & 1) === 0) return { image: (bl & 2) === 0 ? 0x9239 : 0x923b, lift: 0 };
          if ((bl & 2) === 0) return { image: 0x923d, lift: 0 };
          if ((bl & 0x10) === 0) return { image: 0x9239, lift: 0x10 };
          if ((bl & 4) !== 0) return { image: 0x923b, lift: 0x10 };
          if ((bl & 8) !== 0) return { image: 0x923d, lift: 0x10 };
          return { image: 0x9239, lift: 0x10 };
        },
        regs: { al: 0x1f, cl: 0, di: 1, si: 0x1e, ah: 9, e8: 0x1f, ea: 1 },
      },
      {
        img: () => {
          if ((bl & 4) === 0) return { image: (bl & 2) === 0 ? 0x9238 : 0x923c, lift: 0 };
          if ((bl & 2) === 0) return { image: 0x923a, lift: 0 };
          if ((bl & 0x10) === 0) return { image: 0x9238, lift: 0x10 };
          if ((bl & 8) !== 0) return { image: 0x923a, lift: 0x10 };
          if ((bl & 1) !== 0) return { image: 0x923c, lift: 0x10 };
          return { image: 0x9238, lift: 0x10 };
        },
      ...
      {
        img: () => {
          if ((bl & 4) === 0) return { image: (bl & 8) === 0 ? 0x9239 : 0x923d, lift: 0 };
          if ((bl & 8) === 0) return { image: 0x923b, lift: 0 };
          if ((bl & 0x10) === 0) return { image: 0x9239, lift: 0x10 };
          if ((bl & 2) !== 0) return { image: 0x923b, lift: 0x10 };
          if ((bl & 1) !== 0) return { image: 0x923d, lift: 0x10 };
          return { image: 0x9239, lift: 0x10 };
        },
    ...
    for (const corner of corners) {
      ...
      al = (al >>> 1) & 0xff;
      if (carry !== 0) {
        const selected = corner.img();
        const dxPaint = (dxBase + selected.lift) & 0xffff;
        if (under || dxPaint >= water) {
          corner432204(heap, cpu, runFunction, { ...corner.regs, ebx: selected.image }, dxPaint);
        }
      }
      cpu.regs.ebx = savedEbx;
      cpu.regs.edx = (cpu.regs.edx & 0xffff0000) | dxBase;
  ...
  return runCornerSetter(heap, cpu, runFunction);
}
...
function slopeExtraBlock(heap, cpu, runFunction) {
  ...
  const savedEcx = cpu.regs.ecx >>> 0;                       // push ecx
  {
    ...
    cpu.regs.ebp = rot;
    callBridge(heap, cpu, runFunction, heap.u32((TBL_431BB8 + (rot & 3) * 4) >>> 0));
  }
  ...
  cpu.regs.edx = ((cpu.regs.edx & 0xffff0000) | dx) >>> 0;
  callBridge(heap, cpu, runFunction, 0x004219b5);             // water-edge walkers
  callBridge(heap, cpu, runFunction, 0x00421b78);
  callBridge(heap, cpu, runFunction, 0x004210f9);
  callBridge(heap, cpu, runFunction, 0x00421553);
  ...
  // outside its supported targets — the binary would wild-jump there, so fall
  // back to the raw bytes for exactness on any such slope.
  return runCornerSetter(heap, cpu, runFunction);
}

function runCornerSetter(heap, cpu, runFunction) {
  if (globalThis.__forceInterp422a90) return runBodyFrom(heap, cpu, runFunction, 0x422a90);
  ...
  try {
    ...
    cpu.eflags.CF = regs.cf; cpu.eflags.ZF = regs.zf;
    cpu.eflags.SF = regs.sf; cpu.eflags.OF = regs.of;
  } catch (_) {
    return runBodyFrom(heap, cpu, runFunction, 0x422a90);
  }
  return true;
}
...
function callBridge(heap, cpu, runFunction, fnAddr) {
  if (fnAddr === 0) return;
  cpu.eflags.CF = 0; cpu.eflags.ZF = 0; cpu.eflags.SF = 0; cpu.eflags.OF = 0;
  if (callPromotedLiftedFromCpu(heap, cpu, fnAddr)) return;
  if (globalThis.__421dNativeCalls instanceof Map) {
    globalThis.__421dNativeCalls.set(fnAddr >>> 0, (globalThis.__421dNativeCalls.get(fnAddr >>> 0) || 0) + 1);
  }
...
function paintBody421d2c(heap, cpu, runFunction) {
  const cold = (reason) => {
    if (globalThis.__421dCold instanceof Map) {
      globalThis.__421dCold.set(reason, (globalThis.__421dCold.get(reason) || 0) + 1);
    }
    return false;
  };
  ...
  //
  // Flat surfaces have a small selector table at 0x421f38. It is production
  // JS on the real-startup path; an explicit false retains the interpreter
  // oracle for differential debugging and the frozen legacy soak stays put.
  let baseEsiUpper = esi0 & 0xffff0000;
  let flatSkipShade = false;
  if ((e5 & 0xe0) === 0) {
    const useFlatPort = globalThis.__js421dFlat ?? globalThis.__realStartup === true;
    if (!useFlatPort) return cold("flat-tile");

    // 0x421e13 / 0x421e21 both branch to the normal shade block. Otherwise
    // `mov si,[esi+6]; and esi,7` supplies the table selector and clears the
    // upper half of ESI for the base-sprite call at 0x421fe9.
    if (heap.u16(0x005f472a) === 0 && (f8c & 1) === 0) {
      baseEsiUpper = 0;
      const selector = heap.u16(esi0 + 6) & 7;
      if (selector === 0) {
        const rotation = heap.u32(0x00991f88) & 3;
        bx = (bx + heap.u16(ediVal + rotation * 4 + 0x005f4938)) & 0xffff;
        ebx = (ebx & 0xffff0000) | bx;
        flatSkipShade = true;
      } else if (selector === 4 || selector === 5 || selector === 6) {
        const stateIndex = (((heap.u16(0x00991f72) & 0x20)
          | ((heap.u16(0x00991f76) & 0x20) << 1)) >>> 5) & 3;
        const table = selector === 6 ? 0x005f4928 : 0x005f4918;
        bx = (bx + heap.u16(ediVal + stateIndex * 4 + table)) & 0xffff;
        ebx = (ebx & 0xffff0000) | bx;
        flatSkipShade = true;
      } else if (selector === 7) {
        // The final table word overlaps code and is unreachable for valid
        // surface elements. Preserve oracle behaviour if malformed data hits.
        return cold("flat-selector-7");
      }
      // Selectors 1..3 target 0x421fb2 and use the normal shade block.
    }
  }
  // Sloped surfaces and flat selectors 1..3 both execute 0x421fb2.
  ...
  cpu.regs.esi = (baseEsiUpper | 0x0020) >>> 0;                  // si=0x20 (NOT esi0!)
  ...
  callBridge(heap, cpu, runFunction, subFn);
  ...
  if ((f8c & 0x200) !== 0) return cold("overlay-0x200");

  // === 0x422136: test [0x991f8c], 0x400; je 0x4221ca ===
  if ((f8c & 0x400) !== 0) return cold("overlay-0x400");

  // === 0x4221ca: test [0x99a020], 1; je 0x422344 ===
  const a020 = heap.u16(0x0099a020);
  if ((a020 & 1) !== 0) return cold("selected-tile");

  // === 0x422344: test [0x99a020], 2; je 0x4223a6 ===
  if ((a020 & 2) !== 0) return cold("selection-marker");

  // === 0x4223a6: test [0x991f8c], 1; je 0x4223f4 ===
  if ((f8c & 1) !== 0) return cold("overlay-0x1");
  ...
  // === 0x42243c..0x4225b6: copy 16 pairs of dwords ===
  //   src layout: [0x999f9a + 4*i], [0x999fdc + 4*i]   for i = 0..15
  //   dst layout: [0x5f4104 + 4*i],  [0x5f4146 + 4*i]
  //
  // SIXTEEN, not eighteen (ADDENDUM 120). The count is fixed by the unrolled
  // block's first and last instructions:
  //   0x42243c: mov edi,[0x999f9a] / 0x422448: mov [0x5f4104],edi   <- i=0
  //   0x4225b0: mov [0x5f4140],edi / 0x4225b6: mov [0x5f4182],esi   <- i=15
  //   0x4225bc: shr dx,4                                            <- block ends
  // 0x5f4146 + 4*15 = 0x5f4182, so i stops at 15. The port ran i<18 and wrote
  // two pairs the binary never writes, at 0x5f4144/0x5f4148 and
  // 0x5f4186/0x5f418a — and 0x5f4186..0x5f418d is EXACTLY the 8-byte heap
  // divergence in this page. Pixel-invisible (the extra slots are not sampled
  // when rendering this scene), which is why no accuracy gate ever caught it.
  for (let i = 0; i < 16; i++) {
  ...
  regs.edx = dxLocal;
  // `pop esi` (0x4225d5) — esi0 was pushed at 0x421dab. This restore is NOT
  // optional: the four palette helpers above DO clobber cpu.regs.esi (0x420502
  // and 0x42094b both write it, and so do their binary bodies on the
  // interpreter fallback leg). Measured: esi differed from esi0 on 2862/2862
  // calls, including all 102 cliff calls. Everything downstream of here reads
  // esi — most importantly 0x422801's `mov al,[esi+7]`, which on the cliff
  // handoff was reading the edge-corner mask of a DIFFERENT tile element.
  cpu.regs.esi = esi0;
  regs.esi = esi0;
  ...
  if (e7Low !== 0) {
    ...
    // ADDENDUM 119 — the corners are NO LONGER suppressed.
    //
    // The suppression above was justified by ADD 16 "Bug 2": the JS body was
    // said to leave eax/ebx desynced from the binary here, so any corner paint
    // would run off wrong registers. Re-examined instruction by instruction,
    // eax CANNOT matter: 0x422801 is `mov al,[esi+7]` (AL reloaded from
    // memory), and in `shl ax,cl` the old AH shifts LEFT away from AL before
    // `mov ah,al` discards it outright. Only bl/cl/dx/esi are read.
    //
    // Measured, not assumed: handing off to the interpreter AT 0x422801 with
    // this body's own live registers renders 0/307200 px against interpreter
    // ground truth. If the registers were wrong, that would diverge. They are
    // correct, so the corners can simply be drawn.
    //
    // Production real startup uses the JS tail. Preserve the native entry as
    // an explicit oracle leg and keep the frozen legacy soak unchanged.
    const useCliffPort = globalThis.__js421dCliff ?? globalThis.__realStartup === true;
    if (!useCliffPort) return runBodyFrom(heap, cpu, runFunction, 0x00422801);
    return cliffCornerBlock(heap, cpu, runFunction);
  }
  ...
  // The first 16 slots select the existing inline corner-heights setters.
  // Each case writes
  ...
  // ebx; extended slope indices 23, 27, 29 and 30 have distinct bodies,
  // while the other second-half slots alias the flat case. Use the shared JS
  // dispatcher, which resolves the full table address and preserves flags.
  // Unknown targets retain the explicit development interpreter fallback.
  if ((ebx & 0xffff) > 0xf) {
    cpu.regs.ebx = ebx >>> 0;
    return runCornerSetter(heap, cpu, runFunction);
  }
...
function runBodyFrom(heap, cpu, runFunction, addr) {
  if (globalThis.__421dNativeTails instanceof Map) {
    globalThis.__421dNativeTails.set(addr >>> 0, (globalThis.__421dNativeTails.get(addr >>> 0) || 0) + 1);
  }
  const savedESP = cpu.regs.esp >>> 0;
  const savedEIP = cpu.regs.eip >>> 0;
  const savedCallDepth = cpu.callDepth;
