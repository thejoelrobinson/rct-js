...
function FUN_00431510_translated(heap) {
  let bVar1 = 0;
  let psVar2 = 0;
  let sVar3 = 0;
  let sVar4 = 0;
...
export function FUN_00431510_exact(heap) {
  const savedEsi = regs.esi >>> 0;
  const result = FUN_00431510_translated(heap) >>> 0;
  regs.ebx = ((regs.ebx & 0xffffff00) | heap.u8(0x00628910)) >>> 0;
  regs.eax = ((regs.eax & 0xffff0000) | heap.u16(0x00628914)) >>> 0;
  regs.ecx = ((regs.ecx & 0xffff0000) | heap.u16(0x00628916)) >>> 0;
  regs.edx = heap.u32(0x00628918) >>> 0;
  regs.esi = savedEsi;
  return result;
}
...
export function FUN_00431510(heap) {
  if (globalThis.__forceFrozen431510) return FUN_00431510_translated(heap);
  if (globalThis.__realStartup || state.executionMode === "pure-js") return FUN_00431510_exact(heap);
  return FUN_00431510_translated(heap);
}
