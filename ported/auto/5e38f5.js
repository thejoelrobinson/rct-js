...
export function FUN_005e38f5(heap) {
  ...
  // 0x5e38f5: `66 0f ba 35 fc a4 99 00 01` = btr WORD ptr [0x99a4fc], 1.
  // A 16-bit bit-test-and-reset: CF = bit 1, then bit 1 is cleared. Ghidra
  // lowered it to a 32-bit read-modify-write and kept the 16-bit mask 0xfffd,
  // so `u32(0x99a4fc) & 0xfffd` also zeroed bits 16-31 — i.e. the WORD at
  // 0x99a4fe, which is the sprite tick incremented at 4385d8.js:215. This ran
  // once per frame via FUN_004270f2, so the sprite tick was reset to 0 every
  // tick and the ENTIRE simulation stood still: guests never moved, rides
  // never ran, and only 111 of 147,456 sprite bytes changed over 500 ticks.
  uVar1 = ((heap.u16(0x0099a4fc) >>> 1) & 0xffff);
  heap.setU16(0x0099a4fc, (heap.u16(0x0099a4fc) & 0xfffd) & 0xffff);
  if ((uVar1 & 1) != 0) {
