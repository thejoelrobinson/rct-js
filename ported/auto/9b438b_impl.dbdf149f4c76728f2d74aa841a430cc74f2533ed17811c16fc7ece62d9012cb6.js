...
export function FUN_009b438b(heap) {
  ...
  // 0x9b439b `je 0x9b4457`, 0x9b4438 `jmp 0x9b4457`, and the 0x9b443a/0x9b4442
  // tails that run off the end of the prologue at 0x9b4456: EVERY path through
  // 0x9b438b arrives at 0x9b4457, and none of them touches EBX/ECX/EDX/ESI/EDI
  // on the way, so the fall-through is exactly a call to FUN_009b4457 with the
  // registers as they came in.
  //
  // Ghidra INLINED that body here instead, so this file carried a second,
  // independent copy of 0x9b4457 — and it did not receive the left-clip and
  // 16-bit-extent fixes made in 9b4457.js. Sprite 0x95c6 (the scenario-detail
  // window, drawn via 0x429ae4) goes through here, and it never reached
  // 9b4457.js at all: 44 blits in the interpreter, 0 in JS, 6552 divergent px
  // in the scenario-select shot.
  //
  // GATED (CLAUDE.md's gating pattern): the inlined copy below is what the
  // frozen sc21 soak (canary 7b14266) was captured through. Retire the gate
  // when the soak is re-baselined.
