// Auto-translated from Ghidra C by tools/c-to-js/translate.js.
// Source: decompiled/c/44294c.c
// Edit by hand only after diff-test passes — re-running the translator will overwrite.

/** @typedef {import("../runtime/heap.js").Heap} Heap */

import { CONCAT44 } from "../runtime/win32.js";
import { FUN_00423677 } from "./423677.js";
import { FUN_0042de29 } from "./42de29.js";
import { FUN_0043424f } from "./43424f.js";
import { FUN_005e68e2 } from "./5e68e2.js";
export function FUN_0044294c(heap) {
  let sVar1 = 0;
  let in_EAX = 0;
  let in_EDX = 0;
  let bVar2 = 0;
  bVar2 = heap.u32(0x0099a4f6) < 0x8000;
  if (heap.u32(0x0099a4f6) == 0x8000) {
    FUN_005e68e2(heap);
    if (bVar2) {
      /* goto LAB_004429a6 */ throw new Error("goto LAB_004429a6 not supported");
    }
    sVar1 = FUN_0043424f(heap);
    if (sVar1 == -0x8000) {
      /* goto LAB_004429a6 */ throw new Error("goto LAB_004429a6 not supported");
    }
    FUN_00423677(heap);
  }
  FUN_0042de29(heap);
  LAB_004429a6: return CONCAT44(heap, in_EDX, in_EAX);
}
