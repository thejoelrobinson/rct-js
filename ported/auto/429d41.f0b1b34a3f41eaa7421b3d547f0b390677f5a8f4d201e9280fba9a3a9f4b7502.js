...
function handleChartShift(heap) {
  const compare16 = (left, right) => {
    ...
    regs.sf = result >>> 15;
    regs.of = ((left ^ right) & (left ^ result)) >>> 15;
  };
  const compare32 = (left, right) => {
  ...
  for (;;) {
    const live = heap.u16(0x005f54f0 + regs.edi * 2);
    regs.eax = ((regs.eax & 0xffff0000) | live) >>> 0;
    compare16(live, heap.u16(0x005f5518 + regs.edi * 2));
    if (!regs.zf) break;
    incrementEdi();
    compare32(regs.edi, 0x14);
    if (!regs.cf) return;
  }
  ...
  for (;;) {
    const live = heap.u16(0x005f54f0 + regs.edi * 2);
    regs.eax = ((regs.eax & 0xffff0000) | live) >>> 0;
    heap.setU16(0x005f5518 + regs.edi * 2, live);
    incrementEdi();
    compare32(regs.edi, 0x14);
    if (!regs.cf) break;
  }

  regs.eax = ((regs.eax & 0xffff0000) | 0x0682) >>> 0;
  regs.ebx = regs.ebx & 0xffff0000;
  if (globalThis.__realStartup) FUN_005e5301_exact(heap);
  else FUN_005e5301(heap);
}
...
export function FUN_00429d41(heap) {
  ...
  if (bp === 2) {
    const result = (bp - 2) & 0xffff;
    regs.cf = bp < 2 ? 1 : 0;
    regs.zf = result === 0 ? 1 : 0;
    regs.sf = result >>> 15;
    regs.of = ((bp ^ 2) & (bp ^ result)) >>> 15;
