...
export function FUN_0043054e(heap) {
  ...
  // HAND-FIX (Task #15): TWO translator bugs gated the entire load block, so
  // picking a scenario silently did nothing.
  //  1. The scenario id is a 16-bit field — the binary reads it with
  //     `movzx ..., word ptr [0x8dbed2]`. Read as u32 it picks up whatever sits
  //     in the adjacent high word, so `in_AX == id` never matched and every
  //     scenario load fell through to the bare `return`.
  //  2. `bVar11` is FUN_0042fd81's carry ("load failed"), which Ghidra
  //     collapsed to a hardcoded `false` inside the filename-copy loop above.
  //     42fd81 now sets regs.cf explicitly, so read the real flag — otherwise a
  //     failed .SC4 would be treated as a successful load.
  const _loadFailed = (regs.cf & 1) !== 0;
  if ((!_loadFailed) && (in_AX == heap.u16(0x008dbed2))) {
    ...
    // asm 0x430663: mov edx, dword ptr [0x5f8534] / 0x430669: mov dword ptr [0x6e3b88], edx
    // — the STORE is genuinely 32-bit; the LOAD was mis-widthed (u8), so this restored only
    // the low byte of the saved 0x6e3b88 pointer.
