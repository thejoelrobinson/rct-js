...
export function FUN_0042afb5(heap) {
  ...
  if ((regs.edi >>> 0) !== 0xffffffff) {
    ...
    // 0x42b00e..16: widget-id lookup. cl = 0x94 (signed -0x6c). The result
    // is communicated through ZF — FUN_005e3b2b doesn't return it, so we
    // can't reproduce the bit-set without re-running the search. As a
    // close approximation we just call it for side effects (it's a search
    // with no observable globals). Bit 0xb stays cleared, which matches
    // the "no foreground hover" case — visually identical until tooltips
    // ship.
