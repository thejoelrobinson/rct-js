...
function paintBody421d2c(heap, cpu, runFunction) {
  ...
  // ebx. FLAT tiles with e4&0x10 reach here with ebx in 16..31 (savedEbxStack
  // = e4&0x1f), and the jumptable has DISTINCT entries beyond the 16
  // CORNER_CASES (e.g. 0x17 -> 0x4233f3). The inline table only covers 0..15,
  // so delegate ebx>15 to the real jumptable via the interpreter — correct by
  // construction (ADDENDUM 90; sloped tiles never have ebx>15, so their fast
  // path is unaffected). [0x991f2b]|=1 was already done above; 0x422a90 redoes
  // it (idempotent) then jmps.
  if ((ebx & 0xffff) > 0xf) {
    cpu.regs.ebx = ebx >>> 0;
    return runBodyFrom(heap, cpu, runFunction, 0x00422a90);
  }
